...
impl Logger for RttLogger {
    fn trace(&mut self, message: &dyn Display) {
       write!(&mut self.up_channel, "Trace: {}", message).unwrap();
    }

    fn debug(&mut self, message: &dyn Display) {
        write!(&mut self.up_channel, "Debug: {}", message).unwrap();
    }

    fn info(&mut self, message: &dyn Display) {
        write!(&mut self.up_channel, "Info: {}", message).unwrap();
    }

    fn warn(&mut self, message: &dyn Display) {
        write!(&mut self.up_channel, "Warning: {}", message).unwrap();
    }

    fn error(&mut self, message: &dyn Display) {
        write!(&mut self.up_channel, "Error: {}", message).unwrap();
    }
}
