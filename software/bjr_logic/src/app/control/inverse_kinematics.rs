...
fn aligned_state(angle_state: KinState, height_state: KinState, base_circle_radius:f32, hinge_offset:f32, joint_circle_radius:f32, min_height:f32, piston_body_len: f32) -> KinState{
    ...
    let a = height_state.accel*sinb+
        angle_state.speed*angle_state.speed* joint_circle_radius *cosf(beta+angle_state.pos) +
        angle_state.accel* joint_circle_radius *sinbpa;
    KinState{pos:l-piston_body_len,speed:v, accel:a}
}
pub fn inverse_kinematics(plate: &PlateState) -> [KinState;3]{
    ...
    [
        aligned_state((plate.angle[0]*SQ3-plate.angle[1])/2.0, plate.height, base_circle_radius, hinge_offset, joint_circle_radius, min_height, pistion_body_len),
        aligned_state(plate.angle[1], plate.height, base_circle_radius, hinge_offset, joint_circle_radius, min_height, pistion_body_len),
        aligned_state((-plate.angle[0]*SQ3-plate.angle[1])/2.0, plate.height, base_circle_radius, hinge_offset, joint_circle_radius, min_height, pistion_body_len),
    ]
}
#[cfg(test)]
