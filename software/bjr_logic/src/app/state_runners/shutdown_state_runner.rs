...
use crate::app::parameter_manager::{
    parameter_manager, HomingAccel, HomingHighVelocity, HomingSafePosition,
};
    ...
    fn update<LOG: Logger>(
        ...
        if !self.started {
            ...
            if !self.finished {
                ...
                let motors_input = ctx
                    .iomanager
                    .read_motor_inputs(ctx.telemetry_builder)
                    .map_err(|e| StateRunnerError::IOError(e))?;
