...
mod tests {
    ...
    use crate::app::control_primitives::KinState;
    use crate::app::event::GlobEvent;
    use crate::app::io_manager::{IOManagerError, Inputs, Outputs};
    use crate::app::motor_handler::{ControlMode, MotorStatus};
    use crate::app::parameter_manager::{
        parameter_manager, HomingAccel, HomingHighVelocity, HomingLowVelocity, HomingSafePosition,
    };
    use crate::app::state_runner::RunnableState;
    use crate::utils::test_helper::bsp_mocks::{
        MockTestIOManager, MockTestLogger, MockTestMotorEnabler,
    };
    ...
    fn expect_motor_output_safe_pos(mock_iomanager: &mut MockTestIOManager) {
        let expected_motor_output_safe_pos = [Some((
            KinState {
    ...
    fn expect_no_motor_output(mock_iomanager: &mut MockTestIOManager) {
        let expected_motor_output_safe_pos: [Option<(KinState, ControlMode)>; MOTOR_NUM] =
            [None; MOTOR_NUM];
        mock_iomanager
            .expect_write_motor_outputs()
            .withf(|e| true)
    ...
    fn expect_motor_input(mock_iomanager: &mut MockTestIOManager, status: MotorStatus) {
        ...
        let a = mock_iomanager
            .expect_read_motor_inputs()
            .with()
            .times(1)
            .returning(move || Ok(expected_motor_input_none));
    }
    fn expect_motor_update(
        mock_iomanager: &mut MockTestIOManager,
        test_homing_state_runner: &mut HomingStateRunner,
        call_time: u64,
        test_queue: EventQueue,
        test_logger: &mut MockTestLogger,
    ) {
        let result = test_homing_state_runner.update(
            mock_iomanager,
            Microseconds::new(call_time),
            test_queue,
            test_logger,
    ...
    fn test_homing_runner_homing_from_afar() {
        ...
        mock_test_logger
            ...
            .with(always())
            .returning(|_| ());
        mock_test_motor_enabler
            .expect_set_enable()
            .with(eq(true))
            .times(1)
            .returning(|_| {});
        test_homing_state_runner.entry(
            Microseconds(0),
            &mut mock_test_motor_enabler,
            &mut mock_test_logger,
        );
        //update (1000us)
