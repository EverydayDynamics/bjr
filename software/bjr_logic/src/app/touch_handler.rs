...
use crate::app::parameter_manager::{
    parameter_manager, LogicRunnerPeriodUs, TouchCenterOffsetX, TouchCenterOffsetY, TouchScaleXX,
    TouchScaleXY, TouchScaleYX, TouchScaleYY,
};
...
where
    ...
    DIFF: Differentiator,
{
    pub fn new(touch_sensor: TS, differentiators: [DIFF; 2]) -> TouchHandler<TS, DIFF> {
        TouchHandler {
            touch_sensor,
            differentiators,
    ...
    pub fn get_ball_state(
        ...
        if let Some(touch_pos) = self.touch_sensor.get_touch()? {
            ...
            telemetry_builder.add_unfiltered_ball_velocities(&[
                self.unfiltered_differentiators[0]
                    .get_diff(delta_t)
                    .unwrap_or(0.0),
                self.unfiltered_differentiators[0]
                    .get_diff(delta_t)
