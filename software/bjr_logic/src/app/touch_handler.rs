...
{
    pub fn new(touch_sensor: TS, initial_call_time: Microseconds<u64>, initial_state: Option<[KinState;2]>) -> TouchHandler<TS> {
        TouchHandler{
            touch_sensor,
...
mod tests {
    ...
    fn test_recover_from_lost_ball() {
        let expected_touch_x = (0.01 / parameter_manager().get::<TouchScaleX>()) as i32 - parameter_manager().get::<TouchCenterOffsetX>();
        let expected_touch_y = (0.01 / parameter_manager().get::<TouchScaleY>()) as i32 - parameter_manager().get::<TouchCenterOffsetY>();
        let mut mock_touch_sensor = MockTouchSensor::new();
        mock_touch_sensor.expect_get_touch().times(1).returning(move ||Ok(Some(Point{ x: expected_touch_x, y: expected_touch_y})));
        let mut test_touch_handler = TouchHandler::new(mock_touch_sensor, Microseconds::<u64>::new(0), None);
    ...
    fn test_lost_ball() {
        let mut mock_touch_sensor = MockTouchSensor::new();
        mock_touch_sensor.expect_get_touch().times(1).returning(move ||Ok(None));
        let mut test_touch_handler = TouchHandler::new(mock_touch_sensor, Microseconds::<u64>::new(0), Some([KinState{
            pos: 0.1,
            speed: 0.0,
            accel: 0.0,
        },KinState{
    ...
    fn test_continued_ball_find() {
        let expected_touch_x = (0.1 / parameter_manager().get::<TouchScaleX>()) as i32 - parameter_manager().get::<TouchCenterOffsetX>();
        let expected_touch_y = (0.1 / parameter_manager().get::<TouchScaleY>()) as i32 - parameter_manager().get::<TouchCenterOffsetY>();
        let mut mock_touch_sensor = MockTouchSensor::new();
        mock_touch_sensor.expect_get_touch().times(1).returning(move ||Ok(Some(Point{ x: expected_touch_x, y: expected_touch_y})));
        let mut test_touch_handler = TouchHandler::new(mock_touch_sensor, Microseconds::<u64>::new(0), Some([KinState{
            pos: 0.0,
