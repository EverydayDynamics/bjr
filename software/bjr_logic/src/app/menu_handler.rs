...
impl Default for Context {
    fn default() -> Self {
        Context{ _inner: 0,error: Ok(()), state_runner_command: StateRunnerCommand::NoCommand }
    }
}
                    ...
                    item_type: ItemType::Menu(&Menu {
                        ...
                        items: &[
                            &Item{
                                ...
                                item_type: ItemType::Callback { function: set_plate_state, parameters: &[
                                    Parameter::Mandatory { parameter_name: "height", help: Some("the height of the plate to be set in mm")},
                                    Parameter::Mandatory { parameter_name: "alpha", help: Some("the alpha angle of the plate to be set in degrees")},
                                    Parameter::Mandatory { parameter_name: "beta", help: Some("the beta angle of the plate to be set in degrees")},
                                ] },
...
fn fallible_send_event<MIO: Reader+Write>(
    ...
    if let Ok(event) = GlobEvent::from_str(args[0]) {
        ...
        for v in GlobEvent::VARIANTS {
            interface.write_str(v).map_err(|_|MenuError::MenuInterfaceWriteError)?;
            interface.write_str("\n").map_err(|_|MenuError::MenuInterfaceWriteError)?;
        }
...
fn fallible_get_parameter<MIO: Reader+Write>(
    ...
    if let Ok(parameter) = ParameterList::from_str(args[0]) {
        parameter.write_value(interface).map_err(|_|MenuError::MenuInterfaceWriteError)?;
    }else {
...
fn fallible_set_parameter<MIO: Reader+Write>(
    ...
    if let Ok(parameter) = ParameterList::from_str(args[0]) {
        if parameter.set_value(args[1]).is_ok() {
            write!(interface,"{} set to ", parameter).map_err(|_|MenuError::MenuInterfaceWriteError)?;
        } else {
            write!(interface,"Couldn't parse value for {}", parameter).map_err(|_|MenuError::MenuInterfaceWriteError)?;
        }
        parameter.write_value(interface).map_err(|_|MenuError::MenuInterfaceWriteError)?;
...
fn fallible_list_parameters<MIO: Reader+Write>(
    ...
    for param in ParameterList::iter() {
        ...
        param.write_value(interface).map_err(|_|MenuError::MenuInterfaceWriteError)?;
        interface.write_str("\n").map_err(|_|MenuError::MenuInterfaceWriteError)?;
    }
    Ok(())
}
fn enter_feedforward_mode<MIO: Reader+Write>(
    ...
    context.error = get_event_queue().enqueue(GlobEvent::EnterFeedforward).map_err(|_|MenuError::EventBufferOverflow);
}
    ...
    if let Ok(height) = f32::from_str(args[0]) {
       if let Ok(alpha)  = f32::from_str(args[1]) {
           if let Ok(beta)  = f32::from_str(args[2]) {
               ...
               interface.write_str("Couldn't parse Beta angle parameter. Please enter a number.").map_err(|_|MenuError::MenuInterfaceWriteError)?;
           }

       } else {
           interface.write_str("Couldn't parse Alpha angle parameter. Please enter a number.").map_err(|_|MenuError::MenuInterfaceWriteError)?;
       }
    }else {
